...
define(function(require, exports, module){
  ...
  var PULNormalizer = exports.PULNormalizer = function(){
    ...
    this.finalizePUL = function(pul){
      ...
      // Remove non-effective replace-in-object UPs
      var upd = pul.replace_in_object;
      ...
      for (var i = upd.length-1; i >= 0; --i){
        ...
        for (var j = 0; j < delTargets.length; j++){
          if (this.containsTarget(delTargets[j], replaceTarget)){
            upd.splice(i,1);
            break;
          } 
        }
    ...
    this.computeTargets = function(pul){
      ...
      for (var i = 0; i < upd.length; i++){
        targets.insert_into_object.push(this.serializeTarget(upd[i].target));
      }
      upd = pul.delete_from_object;
      for (var i = 0; i < upd.length; i++){
        targets.delete_from_object.push(this.serializeTarget(upd[i].target));
        upd[i].params[0].forEach(function(name){
          targets.delete_from_object_selected.push(target+"."+name);
        });   
      }
      upd = pul.replace_in_object;
      for (var i = 0; i < upd.length; i++){
        var target = serializeTarget(upd[i].target);
        targets.replace_in_object.push(target);
        upd[i].params[0].forEach(function(name){
          targets.replace_in_object_selected.push(target+"."+name);
        });
      }
      upd = pul.rename_in_object;
      for (var i = 0; i < upd.length; i++){
        var target = this.serializeTarget(upd[i].target);
        targets.rename_in_object.push(target);
        targets.rename_in_object_selected.push(target+"."+upd[i].params[0]);
      }
      upd = pul.insert_into_array;
      upd.forEach(function(curUp){
        targets.insert_into_array.push(
          this.serializeTarget(curUp.target)+":"+curUp.params[0]);            
      });
    ...
    this.addUp_insert_into_object = function(pul,up){
      // TODO merge sametarget inserts into one object, error if key conflict
      console.log("Warning: addUp_insert_into_object not implemented");
      pul.insert_into_object.push(up);
    };

    this.addUp_delete_from_object = function(pul,up){
      // Add names to delete-from-object update
    ...
    this.addUp_replace_in_object = 
      this.addUp_rename_in_object = function(pul, up){
        ...
        if (this.findUpdate("replace_in_object", selector, pul[up.type])){
