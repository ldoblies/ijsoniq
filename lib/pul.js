  ...
  var PendingUpdateList = exports.PendingUpdateList = function(handler){
    ...
    this.computeIntroducedTargets = function(){
      ...
      this.insert_into_object.forEach(function(curIIO){
        if (!this.getsDeleted(curIIO.target.collection, curIIO.target.key)){
          var kvPairs = Utils.getKeyValuePairs(curIIO.params[0]);
          kvPairs.forEach(function(curKVPair){
            var item = {};
            item.target = Utils.serializeTarget(curIIO, curKVPair.key);
            item.up = curIIO;
            ret.addItem(item);
          });
        }
      ...
      // 3. Objects introduced by renames (and not deleted by any delete UP)
       this.rename_in_object.forEach(function(curRIO){
        if (!this.getsDeleted(curRIO.target.collection, curRIO.target.key)){
          var item = {};
          item.target = Utils.serializeTarget(curRIO, curRIO.params[1]);
          item.up = curRIO;
          ret.addItem(item);
        }
      });     
      ...
      this.replace_in_object.forEach(function(curRIO){
        ...
        if (typeof replaceValue === 'object' &&
          !this.getsDeleted(curRIO.target.collection, curRIO.target.key)){
          var kvPairs = Utils.getKeyValuePairs(replaceValue);
          kvPairs.forEach(function(curKVPair){
            var item = {};
            item.target = Utils.serializeTarget(curRIO, curKVPair.key);
            item.up = curRIO;
            ret.addItem(item);
          });
        }
