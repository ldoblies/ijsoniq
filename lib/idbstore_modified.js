...
define(function(require, exports, module){
   ...
   */
  var IDBStore = exports.IDBStore = function (kwArgs, onStoreReady) {
  ...
  IDBStore.prototype = /** @lends IDBStore */ {
    ...
    openTransaction: function(objectStores, mode) {
      console.log("IDBStore: opening transaction on", objectStores, ",", mode);
    ...
    put: function (key, value, onSuccess, onError, transaction) {
      ...
      var putTransaction = transaction;
      if (!putTransaction){
    ...
    get: function (key, onSuccess, onError, transaction) {
      ...
      getRequest.onsuccess = function (event) {
        ...
        if (transaction){
          //console.log("IDBstore returning from get():", resultCopy);
          onSuccess(result, transaction);
        }else{
          //console.log("IDBStore: get() without transaction, result: ", result);
        }
      };
    ...
    batch: function (dataArray, onSuccess, onError) {
      ...
      batchTransaction.oncomplete = function () {
        console.log("batchTransaction.oncomplete");
        var callback = hasSuccess ? onSuccess : onError;
        callback(hasSuccess);
      };
      ...
      dataArray.forEach(function (operation) {
        ...
        if (type == "remove") {
          ...
          if (!value){
            onItemError("value is undefined");
          }else{
          var putRequest;
          if (this.keyPath !== null) { // in-line keys
            this._addIdPropertyIfNeeded(value);
    ...
    _addIdPropertyIfNeeded: function (dataObj) {
      if (!this.features.hasAutoIncrement && typeof dataObj[this.keyPath] == 'undefined') {
        if (this._insertIdCount[this.storeName] === undefined){
          this._insertIdCount[this.storeName] = 0;
        }
        dataObj[this.keyPath] = this._insertIdCount[this.storeName]++ + Date.now();
      }
    },
