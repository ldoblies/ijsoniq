  ...
  var PULComposer = exports.PULComposer = function(){
    ...
    this.addUp_del = function(pul,up){
      ...
      if (curDel){
        ...
        curDel.params[0] = curDel.params[0].concat(upIds); }else{ 
          if (!up.delIds2){
            up.delIds2 = {};
          } 
    ...
    // Aggregate up on targetUp. 
    // Returns true iff up was updated and still has to be added to the pul.
    this.aggregate = function(pul, targetUp, up){
      if (targetUp.type === "rename_in_object"){
        if (up.type === targetUp.type){
          var upPathLength = Utils.pathLength(up.target);
          var targetUpPathLength = Utils.pathLength(targetUp.target);
          if (upPathLength === targetUpPathLength){
            // Sequential renaming of the same object: Adapt targetUp
            targetUp.params[1] = up.params[1];            
            return false;
          }else if (upPathLength > targetUpPathLength){
            // Adapt the path in up to point to the original name 
            if (!Utils.setPathEntry(up, targetUpPathLength, targetUp.params[0])){
              // TODO error
              return false;
            }
            return true;
