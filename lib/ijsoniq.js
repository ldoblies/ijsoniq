  ...
  var IJSONiq = exports.IJSONiq = {
    ...
    insert: function(name, items, onSuccess, onError, store, transaction){
      ...
      var applyInserts = function(target, items, onSuccess, onError, transaction, invertedPul){
        ...
        var curItemId = items[0].id;
        var onGetSuccess = function(event){
          var obj = event ? event.target.result : undefined;
          if (obj !== undefined){
            // Error: Document was already in database
            return onError("Document with id '" + curItemId + 
        ...
        if (curItemId){
          var getRequest = transaction.objectStore(target.collection).get(curItemId);
          getRequest.onsuccess = onGetSuccess;
          getRequest.onerror = onError;
        }else{
          onGetSuccess(undefined);
        }
      };
